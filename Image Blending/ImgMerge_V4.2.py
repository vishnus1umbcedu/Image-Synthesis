...
if __name__ == "__main__":
 ...
 #Constructing Image with seam
    for wid in range (0,272):
        for len in range (0,h1):
            if finlist[wid] != 20000:
                imgnew[finlist[wid],wid] = (0,0,255)
            if len < finlist[wid]:
    ...
    #Writing the merged image with seam to output path
    ...
    cv2.imwrite((filepath+"/mergedimage_seam.jpg"),croppedresult)
    filepath = filepath.replace("/","\\")
    print("Find the output image with seam here: "+filepath+"\mergedimage_seam.jpg")
